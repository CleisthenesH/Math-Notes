...
\begin{tikzpicture}[every node/.style={black}]
	...
	\begin{scope}[-stealth]
		\draw (O) node[below right]{$O$}-- (A) node[right] {$A$};
		\draw (O) -- (A') node[left] {$A'$};
		\draw (O) -- (B) node[above] {$B$};
...
This result is the main reason I was thinking about the Orthogonal group.
\\
...
\subsubsection{Geometry Argument:}
...
\begin{tikzpicture}[every node/.style={black}]
	\draw[dashed,gray] (O) circle (3);
	\coordinate (A') at (-0.798766026,-2.891707599);

	\draw[red] (-3.1,0) node[left]{$R_1$} -- (3.1,00);
	\draw[red] (-1.877669588,2.466648925) node[left]{$R_2$}-- (1.877669588,-2.466648925);

	\begin{scope}[-stealth]
		\draw (O) -- (A) node[right] {$A$};
		\draw (O) -- (A') node[below left] {$A'$};
	\end{scope}
	\pic[draw, -stealth] {angle = A--O--A'};
