...
\begin{tikzpicture}[every node/.style={black}]
	\begin{scope}
		...
		\draw (-4,0) -- (4,0);
	\end{scope}
	\draw[red,dashed] plot[smooth,domain=-4:4] (\x, {2-0.125*\x*\x});
	\draw[red,dashed,stealth-stealth] (-4,4)--(4,4);
	\draw (-1,1.875) circle (1.875);

	\coordinate (F) at (0,0);
	\coordinate (T) at (-1,4);
	\coordinate (P) at (-1,1.875);
	\coordinate (A) at (-1,0);
	\coordinate (B) at (-1.8823529,3.52941176);

	\draw[blue] (T) node[above] {$T$} --
	(P) node[above right] {$P$} --
	(F) node[below] {$F$};

	\draw[green] (A) node[below] {$A$} --
	(P) --
	(B) node[above left] {$B$};
\end{tikzpicture}
\end{center}
...
Let $F$ be the center of the semicircle and $P$ be the center of interior circle.
